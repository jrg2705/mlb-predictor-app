...
const ConfidenceBadge = ({ pct }) => {
  const color = pct >= 68 ? "#2D6A4F" : pct >= 55 ? "#F4A261" : "#c0392b";
...
export default function MLBPredictor() {
  ...
  // Navegación: atrás del teléfono → Predictor (inicio)
  const goToTab = (nextTab) => {
    if (nextTab === tab) return;
    if (nextTab !== "predictor") {
      window.history.pushState({ mlbTab: nextTab }, "");
    } else if (tab !== "predictor") {
      window.history.pushState({ mlbTab: "predictor" }, "");
    }
    setTab(nextTab);
  };
  ...
  const [expertPicksCount, setExpertPicksCount] = useState(3);
  ...
  const handleGenerateExpertPicks = async () => {
    ...
    try {
      const res = await fetch("/api/expert-picks", {
        ...
        body: JSON.stringify({
          games: todayAnalyzed,
          pickCount: expertPicksCount,
          calibration: getCalibrationSummary(trackRecord).forExpert,
        }),
      });
      const data = await res.json();
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: "8px", justifyContent: "center", marginBottom: "24px", flexWrap: "wrap" }}>
        <TabButton active={tab === "predictor"} onClick={() => goToTab("predictor")}>⚾ Predictor</TabButton>
        <TabButton active={tab === "today"} onClick={() => goToTab("today")}>📅 Partidos de Hoy</TabButton>
        <TabButton active={tab === "history"} onClick={() => goToTab("history")}>🕓 Historial ({history.length})</TabButton>
        <TabButton active={tab === "track"} onClick={() => goToTab("track")}>🎯 Track Record</TabButton>
        <TabButton active={tab === "picks"} onClick={() => goToTab("picks")}>🍀 Top Picks</TabButton>
        <TabButton active={tab === "standings"} onClick={() => goToTab("standings")}>🏆 Posiciones</TabButton>
        <TabButton active={tab === "h2h"} onClick={() => goToTab("h2h")}>🔁 H2H</TabButton>
      </div>
          ...
          {trackRecord.total === 0 ? (
            ...
            <>
              ...
              {(() => {
                ...
                return (
                  <div style={{ background: "#142235", border: "1px solid #F4A261", borderRadius: "12px", padding: "20px", marginTop: "14px" }}>
                    <div style={{ fontSize: "11px", color: "#F4A261", letterSpacing: "0.15em", marginBottom: "10px" }}>
                      📐 CALIBRACIÓN ACTIVA (se actualiza sola con cada verificación)
                    </div>
                    <p style={{ fontSize: "12px", color: "#c5d8ea", margin: "0 0 10px", lineHeight: 1.5 }}>
                      Esto convierte tu Track Record en reglas de picks: Top Picks y Picks Expertos ya usan estas prioridades.
                    </p>
                    <ul style={{ margin: "0 0 10px", paddingLeft: "18px", fontSize: "12px", color: "#F0F4F8", lineHeight: 1.6 }}>
                      {cal.advice.map((line, i) => (
                        <li key={i}>{line}</li>
                      ))}
                    </ul>
                    {cal.preferLabels.length > 0 && (
                      <p style={{ fontSize: "12px", color: "#2D6A4F", margin: "0 0 6px" }}>
                        ✅ Priorizar: {cal.preferLabels.join(", ")}
                      </p>
                    )}
                    {cal.avoidLabels.length > 0 && (
                      <p style={{ fontSize: "12px", color: "#c0392b", margin: 0 }}>
                        ⛔ Evitar: {cal.avoidLabels.join(", ")}
                      </p>
                    )}
                  </div>
                );
              ...
              <p style={{ textAlign: "center", fontSize: "11px", color: "#3a5a78", marginTop: "14px" }}>
                ...
                Muestras pequeñas (menos de 30-50 partidos) no son representativas. Recalibra solo mirando este panel cada 1–2 semanas.
              </p>
            </>
          )}
        </div>
            ...
            <>
              <div style={{
                ...
                <div style={{ fontSize: "11px", color: "#4A90D9", letterSpacing: "0.15em", marginBottom: "10px" }}>
                  ¿CUÁNTOS PARTIDOS? · UMBRAL DE CONFIANZA
                </div>
              ...
              {generatedPicks && generatedPicks.length === 0 && (
                <p style={{ textAlign: "center", color: "#F4A261", fontSize: "13px", padding: "20px 0" }}>
                  Ningún mercado alcanzó el umbral de {minConfidence}%. Baja el umbral o analiza más partidos.
                </p>
              )}
            </>
          )}
